...
RESULT_DIRECTORY = "/home/jamesdi1993/workspace/Distributed-Sparse-GP/results/nclt"
...
def plot_metrics(num_agents, metrics, metric_name, window_evaluate, save_fig, output_dir, legend_loc=1):
    ...
    if metric_name == "Number of Pseudo":
        ...
        plt.gca().yaxis.set_major_formatter(FormatStrFormatter("%.1fm"))
...
def set_up_figure(grid_min, grid_max):
    ...
    ax.set_xlim(-grid_max[1], -grid_min[1])
    ax.set_ylim(grid_min[0], grid_max[0])
    # ax.set_xlim(-800, 150)
    # ax.set_ylim(-600, 200)
...
def plot_tsdf_horizontal(ax, points, mean, truncated_dist,
              grid_min, grid_max, num_row, num_col):
    zz = mean.reshape(num_row, num_col, order='C').transpose()
    img = ax.imshow(zz, cmap='Purples_r', interpolation='nearest', \
                    extent=[-grid_min[1], -grid_max[1], grid_min[0], grid_max[0]],
                    vmin=-truncated_dist + eps , vmax=truncated_dist - eps, origin='lower')
    plt.axis('off')

def plot_single_tsdf(fig, ax, points, mean, truncated_dist,
              grid_min, grid_max, num_row, num_col, add_color_bar, add_scale_bar):
    zz = mean.reshape(num_row, num_col, order='C').transpose()
    img = ax.imshow(zz, cmap='Purples_r', interpolation='nearest',
                    extent=[-grid_min[1], -grid_max[1], grid_min[0], grid_max[0]],
                    vmin=-truncated_dist + eps, vmax=truncated_dist - eps, origin='lower')
    ...
    if add_color_bar:
        ...
        cbar_tsdf = fig.add_axes([0.825, 0.11, 0.02, 0.77])
        cbar_tsdf.tick_params(labelsize=30)
...
def main():
    ...
    #== load trajectories==#
    trajectory_dir = path.join(dataset_path, "sequences")
    trajectory_format = trajectory_dir + "/{0}_seq_{1}.npz"
    trajectories = []
    for date in dates:
        traj_file = trajectory_format.format(date, seq)
        seq_data = np.load(traj_file)["arr_0"] # x, y
        traj = seq_data[:end, num_hits + 1: -1]
        print("Shape of traj: %s" % (traj.shape, ))
    ...
    fig_traj, ax_traj = set_up_figure(grid_min, grid_max)
    plot_trajectories(ax_traj, trajectories, n_agents=num_agents)
    if save_fig:
        output_file = output_directory + "/trajectories_all.png"
        plt.savefig(output_file)

    fig_tsdf, ax_tsdf = set_up_figure(grid_min, grid_max)
    central_mean = final_agents_pred[-1, :]
    plot_tsdf_horizontal(ax_tsdf, points, central_mean, truncated_dist,
              grid_min, grid_max, num_row, num_col)

    if save_fig:
        output_file = output_directory + "/agent_{0}_horizontal.png".format("central")
        plt.savefig(output_file)

    # Figure for separate TSDF Evaluation
    agent_indices = list(range(10))
    for i, agent_index in enumerate(agent_indices):
        mean = final_agents_pred[agent_index, :]
        print("Shape of prediction: %s" % mean)

        ## Plot TSDF at the last timestamp
        # fig, ax = set_up_figure(grid_min, grid_max)

        fig, ax = set_up_figure(grid_min, grid_max)
        plot_trajectory(ax, trajectories[agent_index], agent_index, n_agents=num_agents)
