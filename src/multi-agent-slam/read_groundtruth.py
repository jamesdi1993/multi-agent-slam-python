...
def main(gt_path, cov_path):

    gt = np.loadtxt(gt_path, delimiter = ",")
    cov = np.loadtxt(cov_path, delimiter = ",")

    print("Ground truth shape: %s" % (gt.shape,))
    print("Cov shape: %s" % (cov.shape,))
    ...
    plt.scatter(y_interp, x_interp, 1, c=-z_interp, linewidth=0)  # Note Z points down
    plt.axis('equal')
...
if __name__ == '__main__':
    date = "2013-04-05"
    gt_path_format = "/home/jamesdi1993/datasets/NCLT/{}/groundtruth_{}.csv"
    cov_path_format = "/home/jamesdi1993/datasets/NCLT/{}/cov_{}.csv"
    main(gt_path_format.format(date, date), cov_path_format.format(date, date))